...
% hack #1: use of the non-idiomatic construct of a cascading 'is' checks
...
% hack #2: interface solution, awkward/complex to use interfaces (the types that we are not in charge of may need wrapping), restrictions on type (no need to wrap the types of others [e.g., primitive types], need for restrictions on multiple interfaces, constructor availability)
Another option would be to rely on interfaces and polymorphism. In this solution, an interface would be used instead of the parameterized type, and that interface would define all the functionality that our class relies on. Any class that we would want to use would then have to implement this interface. This becomes awkward when we are not in charge of the types that we want to use, as they then need wrapping.
