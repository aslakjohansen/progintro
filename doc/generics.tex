...
% problem (knowing that a field exists does know allow us to do much with it, we can only rely on functionality defined on Object, unless we make use of the non-idiomatic construct of a cascading 'is' checks), interface solution, awkward/complex to use interfaces (the types that we are not in charge of may need wrapping), restrictions on type (no need to wrap the types of others [e.g., primitive types], need for restrictions on multiple interfaces, constructor availability)
