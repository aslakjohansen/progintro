...
\begin{itemize}
  \item \idx{Macros}{Macro} can be defined with \idx{\directivename{define}}{define@\directivename{define} directive}. The process of replacing every occurrence of them is referred to as \idx{macro expansion}{Macro!Expansion}.
...
% 1st example: lets look at two examples, in the first example three definitions are involved (DEFAULT, RADIUS2DIAMETER and MULTIPLIER), the two first are defined in the source itself, one is parameterized and one is not, the last definition is provided compiletime as an instruction to the compiler
