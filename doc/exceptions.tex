...
% consequences: the program tells us that the average of 1,2,3 and 5 is 2, but this is wrong! it should be (1+2+4+5)/4=3, but but because of the exception this program calculates it as (1+2+5)/4=2, now this is fairly artificial; no human would write code like this, but take one step back and you should realize that what we have is that while we do catch the exception it still wrecks havoc in terms of the consistency of the variables involved in the loop, in this case it is fairly simple what is going on, but imagine a try clause of 20 line and a handful of function calls, it likely won't be obvious where the exception came from and thus which state modifications have taken effect
