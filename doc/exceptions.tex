...
It makes sense to think of a try construct as a means for us to try out something. But, if something goes wrong then it doesn't roll back: Any side-effects caused from within the try block will \textsl{spill out}. Accordingly, we may end up with side-effects of a partially evaluated \keywordname{try} block, and that can cause inconsistencies in our data if we are not careful.
