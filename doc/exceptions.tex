...
% locating the site of handling: imagine that throughout the execution of our code we keep track of all the handling sites that we come across, whenever an exception is thrown it is caught by the by the code of the latest handling site, execution immediately jumps to this point in the code (even if it is not part of the current function, in that case the environment of that function is reestablished from the stack)
