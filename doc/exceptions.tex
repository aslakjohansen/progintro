...
We have been focusing on the specific type of exception called \typename{Exception}. But the truth is that there are more than one exception type. As they all behave the same, there is little need to dive deeper into them at this point. For now, we will simply -- in figure \ref{fig:exceptions:multiple} -- show how we can have type-specific handling sites. In section \ref{sec:inheritance:exceptions}, we will explore \textsl{why} all exceptions behave the same, and figure out that the order of the catch sites matter and why that is. When executed, this program prints:
...
% note
The code gives a warning because the compiler can see that we are following a \keywordname{null} reference in case 1. This warning has been removed for brevity.
