...
% intro: sometimes we may want to be know whether an excepton has happened without having to deal with it, notice how when we caught an exception we got a variable of type \typename{Exception} and how that was exactly what we threw? Well, we can throw it again, so we catch the exception react to it and throw it again, the code looks a bit different that you would assume:
