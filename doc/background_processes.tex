...
Technically, there are a few other ways of communication available to the pair of processes. Alle processes have a unique process id. The parent process will know both the process id of itself and that of the of child. Throgh a process if, you can send a signal to that process. \defi{Signals}{Signal} are typed and have no payload, so they can really only be used to flag events. Processes can also communicate through files: One process writes to it while anonther reads from it. Lastly, processes can communicate through sockets. We will skip sockets completely in this book, but they are the fundamental abstraction through which internet communication flows.
