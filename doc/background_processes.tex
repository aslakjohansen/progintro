...
% explain it
Figure \ref{fig:bg:processes:edit} illustrates how a typical \idx{text editor}{Text editor} with work with a file. First the file is \idx{opened}{File!Opening}. Then, the contents of the file is read and written to \idx{primary memory}{Memory!Primary}. The user may then manipulate the in-memory representation through some user interface, and saved the modified buffer (memory representation) to disk. Finally, the program will \idx{close}{File!Closing} the file.
