...
We will start this section with a minor digression. In mathematics, the \defi{Fibonacci sequence}{Fibonacci sequence} was originally described by Indian mathematicians (e.g., by \idx{Acharya Pingala}{Pingala, Acharya पिङ्गल} -- पिङ्गल -- around 200 BC\cite{towards_a_global_science}) but today we know it through the name of Fibonacci (aka \idx{Leonardo of Pisa}{Leonardo of Pisa}) who rediscovered it in 1202. This particular sequence appears unexpectedly often in mathematics and nature, and is related to the \idx{golden ratio}{Golden ratio}. It has even found its way into music\cite{tool_lateralus}. Formally, we can define the sequence as:
...
Looking at the above definition, there is no obvious way of implementing the \funcname{fib} function using branches, loops and our basic datastructures. But once you \textsl{get} the idea it is trivial to do so using function calls. That is, function calls to itself. One way of doing so looks like this:
...
There is a one-to-one mapping between the mathematical formula and this implementation. A \keywordname{switch} construct is used to differentiate between the three different cases of the formula. The \textsl{default} matches $n>$ as $n=0$ and $n=1$ have been covered by previous cases and \varname{n} is of an \idx{unsigned}{Integer!Unsigned} integer type: It can never represent a negative number.

% pattern: there is an underlying pattern here: one or more base cases and a recursion step
