...
% fibonacci: we will start this section with a minor deviation, in mathematics the fibonacci sequence is a sequence of integer numbers where each element is the sum of the two previous ones, the sequence was originally described by indian mathematicians (e.g., Acharya Pingala [पिङ्गल] around 200 BC) but today we know it through the name of fibonacci (Leonardo of pisa) who rediscovered the sequence in 1202, this particular sequence appear unexpectedly often in mathematics and nature, and is related to the golden raton, it has even found its way into music, formally we can define the sequence as
\cite{tool_lateralus}\cite{towards_a_global_science}
