...
We have two applications of the pipe operator. This creates a \say{pipeline}. Fist, the expression \valuename{11} is given as argument to the first call to \varname{incr}. This evaluates to $11+1=12$. That 12 is then given as input to the second call to \varname{incr} which evaluates to $12+1=13$.
...
Again, we have a pipeline in three phases. Input is a list with all integers from one to ten (both ends included). In the next phase, the \funcname{map} function is used to create a new list of the same size where the \funcname{incr} function is applied to each of the corresponding values in the input list. Finally, we inspect the result by printing it out. Due to a leftover oddity from Erlang, printing out a list of smal lintegers is overly complicated. Just accept that this is what the last line does.
