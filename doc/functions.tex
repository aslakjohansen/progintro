...
\idx{Python}{Python programming language} is a \idx{scripting language}{Language!Scripting}. 
...
There are a number of side-effects from functions being values. One such effect is that a function can take functions as parameters and have a function as return value. It can also construct a new function dynamically and return it. Functions that have other functions as part of their parameters or return values are called higher-order functions. This property has resulted in a number of patterns to be defined. Let's start with the map pattern. In this pattern a function is applied to each element of a list to construct a new list. In action, it looks like this:

% example: the map function

% filter and reduce: higher-order functions are composable, the map function should be seen as a pattern where something is done to every element of a list, there are other such patterns, the filter pattern constructs a new list that retains all elements of an input list for which some function evaluates to true, explain reduce, the following is a more complex example that illustrate how expressive these can be:
