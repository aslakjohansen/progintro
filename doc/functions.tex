...
% functional
Functional languages, on the other hand, are designed with recursion in mind. They typically optimize for tail recursion. That is, if the last thing that happens in a function is that another function is called and that the return value of this function just needs to be returned, then we don't actually need to place a stack frame on the stack. Most functional languages exploit this for significant efficiency gains. The downside of this is that since stack frames are not being placed on the stack, then the stack doesn't reliably represent which functions have called which functions, and that can make it harder to debug certain problems.
