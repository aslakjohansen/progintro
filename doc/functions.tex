...
Functions -- especially when combined with data structures -- can be used to build higher-level abstractions. One example of this is the linked list from section \ref{sec:primdata:sequence:list}. Figure \ref{fig:fun:case:list} implements a linked list of integers with support for a number of convenient operations. These are:

\begin{itemize}
  \item \funcname{index}: Return the element at an arbitrary position.
  \item \funcname{prepend}: Return a list that creates a new list combining a new \say{link} with a specific payload and an existing list.
  \item \funcname{head}: Returns the value at the head of the list.
  \item \funcname{tail}: Returns the tail of the list.
  ...
  \item \funcname{print}: Print all elements of a list.
\end{itemize}
