...
There is a one-to-one mapping between the mathematical formula and this implementation. A \keywordname{switch} construct is used to differentiate between the three different cases of the formula. The \textsl{default} matches $n>$ as $n=0$ and $n=1$ have been covered by previous cases and \varname{n} is of an unsigned integer type: It can never represent a negative number.
