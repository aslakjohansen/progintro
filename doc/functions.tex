...
Elixir is an example of a functional language. One of the key element of the functional paradigm is that we have access to anonymous functions. Such functions are anonymous is the sense that the are not assigned a static name thought which they can be called. But how can we call them if they are nameless? Well, in functional languages, functions are values and can thus be assigned to variables. Those variable have names, and throught those names we can call them:
