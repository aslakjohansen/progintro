...
Again, we have a pipeline in three phases. Input is a list with all integers from one to ten (both ends included). In the next phase, the \funcname{Enum.map} function is used to create a new list of the same size where the \funcname{incr} function is applied to each of the corresponding values in the input list. Finally, we inspect the result by printing it out. Due to a leftover oddity from Erlang, printing out a list of smal lintegers is overly complicated. Just accept that this is what the last line does.
