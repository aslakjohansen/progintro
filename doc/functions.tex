...
Let's first identify what is present in the program: A single global variable is defined, then comes two functions and finally the \say{main} code that calls one function after the other. The first function is called \funcname{incr} and it increments to value of the variable \varname{value}. It is allowed to do so, as the value is in global scope. Updating a variable of a different scope than that of the function is a side-effect. The second function takes an integer as a parameter. It then prints out a number of dashes that corresponds o that parameter, and finally it breaks the line. Each of these printouts constitute a side-effect.
