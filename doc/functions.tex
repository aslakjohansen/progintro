...
% reusable part: index
All functions have been prefixed \funcname{intlist\_} to avoid naming conflicts. The implementation of \funcname{intlist\_index} employs two guards. The first one on line 13 guards against having to index into a \valuename{null} reference. In this case, we return the value \valuename{-1} to indicate that something has gone wrong. In section \ref{sec:exceptions}, we will explore a better solution. On line 14 we check if the current chain link is the one we are looking for and if so, we return the payload. On line 15 we recurse: The call is repeated, only with a smaller problem (the next chain link and a smaller index).

% reusable part: prepend, head and tail
The \funcname{intlist\_prepend} function simply creates a new chain link element with the right values and returns it. The implementations of \funcname{intlist\_head} and \funcname{intlist\_tail} are very similar: As we are returning the value of a field of the chain link parameter, we need to make sure that this is not \valuename{null}. If it is \valuename{null} then a dummy value (that indicates an error) must be returned. Note how this is not the case for \funcname{intlist\_prepend} as \valuename{null} represents the empty list and does thus not represent a special case.
