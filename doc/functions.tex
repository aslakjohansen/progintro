...
\idx{Python}{Python} is a \idx{scripting language}{Language!Scripting}. As is common with such languages, there is no separate compilation phase and \idx{function lookup}{Function lookup} is \idx{dynamic}{Function lookup!Dynamic} (i.e., it happens at \idx{runtime}{Runtime}). Because of this, if we are missing a function declaration for a specific call, that will be discovered when we try to evaluate that call. This results in a \idx{runtime error}{Error!At runtime}, and it can take some time and specific inputs for it to appear.
