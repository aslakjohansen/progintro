...
% setup
To illustrate how the stack is used in practice, we implement and call a palindrome checker. A palindrome checker is a peice of functionality that checks whether something is a palindrome. Any sequence that wouldn't change if reversed is a palindrome. So, the main input is a sequence. For the output we need a type that can have one of two values; either it is a palindrome, or it is not. This is a textbook example of when to use a \typename{bool} value. Now that we understand the interface of the functionality (inputs and outputs), it can be packaged as a function. That function can be implemented in many ways. In \csharp, this would usually be implemented using a loop in order to avoid performing work on the stack. That, however, would not be very helpful for us to illustrate how the stack works. Instead, we go for this recursive solution:
