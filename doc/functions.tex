...
\begin{enumerate}
  ...
  \descitem{Side-Effect Interface:} The side-effect interface is an umbrella term that covers all other interactions. This includes modifications to state external to the function and printouts to the screen. % TODO: add ref to relevant section
\end{enumerate}
...
% explain
In the first three declares one of the simplest possible functions that returns a value. Whenever it is called, it returns the value 42 as an \typename{int}. This stems from line two that contains a return statement. On line five, it is called. When executed, the program thus prints out \say{The answer is 42}.
...
\inputminted[]{csharp}{../src/csharp/functions/void/Program.cs}
