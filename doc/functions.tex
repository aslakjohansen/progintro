...
Recall that definition of $f(x)$ from section \ref{sec:func:linear}? In order to create a \csharp\ equivalent, we need to map $x$ and $f(x)$ to specific types. Let's decide that both are \typename{double}. Then the \csharp\ implementation looks like this:

% example: linear function
\inputminted[]{csharp}{../src/csharp/functions/f/Program.cs}
...
% intro: recap of what a mathematical function is, figure \ref{fig:fun:anatomy} exemplifies the structure of the programming equivalent, we will take some time to go through these in detail
