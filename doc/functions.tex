...
What does the \csharp\ equivalent look like in code? Let us start by defining the interface of the function that we are about to write. It should be obvious that it takes one parameter (the $n$). That $n$ refers to the index of a sequence, and that indicates that it is an unsigned integer. This unsigned integer constitutes the input interface of our function. The output is zero (some integer), $n$ (unsigned integer as input) or the result of adding two of these. Adding an unsigned value and a zero (or another unsigned integer for that matter) will never yield a negative value, and so the output interface must be an unsigned integer.  There does not appear to be a need for it to print out anything, and it doesn't operate on state besides its input. That means that the side-effect interface is empty. So, we are looking for a function with the prototype:
\begin{center}
