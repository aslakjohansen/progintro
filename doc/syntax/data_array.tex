...
\begin{syntax}[[xshift=24mm]concept.west]{arr-decl}
  \SyntaxWestSplit{MainWest}
  \SyntaxEastSplit{MainEast}
  
  \node[sequence,column sep=1.1cm] () at ([yshift=-0*\syntaxruledist]$(begin)!0.525!(end)$) {
    \node[terminal]       (ruleIa) {[};
    &
  ...
  \draw[path] (begin)--(ruleIa)--(ruleIb)--(ruleIc)--(end);
  \draw[path] (ruleIb)
            -|([xshift= 0.7*\syntaxruledist,yshift=0.4*\syntaxruledist]ruleIb.east)
            |-(                            [yshift=0.0*\syntaxruledist]ruleIIa)
            -|([xshift=-0.7*\syntaxruledist,yshift=0.4*\syntaxruledist]ruleIb.west)
            |-(ruleIb);
  \draw[path] (ruleIa)
            -|([xshift=-0.3*\syntaxruledist,yshift=-0.5*\syntaxruledist]ruleIb.west)
            |-(                            [yshift=0.0*\syntaxruledist]ruleIIIa)
            -|([xshift= 0.3*\syntaxruledist,yshift=-0.5*\syntaxruledist]ruleIb.east)
            |-(ruleIc);
\end{syntax}
