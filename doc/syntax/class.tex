...
\begin{syntax}[[xshift=36mm]concept.west]{context-variable}
  \SyntaxWestSplit{MainWest}
  \SyntaxEastSplit{MainEast}
  
  \node[sequence] () at ([yshift=0*\syntaxruledist]$(begin)!0.5!(end)$) {
    \node[nonterminal]    (ruleIa) {context-var-anno};
    &
    \node[nonterminal]    (ruleIb) {local-var};
    \\
  };
...
%  
  ...
  \draw[path] (begin)--(ruleIa)--(ruleIb)--(ruleIc)
    -|([xshift=1cm,yshift=-0.4cm]ruleIc.east) to[|-|] ([xshift=-1cm,yshift=0.4cm]ruleIi.west)|-
