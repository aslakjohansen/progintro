...
This grammar defines five grammatical concepts, namely \textcolor{teal}{pgm}, \textcolor{teal}{stmt}, \textcolor{teal}{param-list}, \textcolor{teal}{expr} and \textcolor{teal}{name}. Each of these definitions define the valid sequences of tokens from an empty circle on the left to a filled circle on the right. We read these by following the tracks. The program itself (or \textcolor{teal}{pgm}) thus has to contain a sequence of at least one statement (or \textcolor{teal}{stmt}). This, due to the loop around \textcolor{teal}{stmt}. % TODO: chose a color for the concepts that doesn't clash with index entries

% stmt concept, param-list concept
The next rule unfolds what a statement is. A statement must consist of a name followed by a period followed by another name followed by a left parenthesis followed by a parameter list (or \textcolor{teal}{param-list}) followed by a right parenthesis followed by a semicolon. The next rule states that this \textcolor{teal}{param-list} is either empty, or a sequence of expressions (\textcolor{teal}{expr}) with commas in between. The \textcolor{teal}{expr} has to be a string literal (\textcolor{teal}{string-lit}).
