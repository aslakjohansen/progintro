...
% transition to REPL
As with the Pytho script (in section \ref{sec:first:python}) it uses a shebang mechanism. For that mechanism to work it needs to be \idx{executable}{Executable}. Hoever, while the interpreter for the script is called \commandname{elixir} this is not the case for the REPL. The interpreter for the REPL is called \commandname{iex}:
...
iex(2)> 
\end{minted}
The line that makes the printout appear is still the same. Here, you see it appear after a prompt. The \commandname{iex} prompt is, unlike the pyton one numbered. Also, everything in Elixir evaluated to a value, and the REPL prints out this value after having evaluated a line. In this case \texttt{IO.puts("Hello, World!")}, after having performed its function of printing \quoted{Hello, World!} to the screen, evaluates to \texttt{:ok}.
