...
% instruction set
These instructions are described through RV32I which is what is called an instruction set architecture (ISA). An ISA specified the programmable interface of a class of processors in an implementation generic way. This includes the binary encoding of each instruction as macine code. A major implication of this is that a programmer can create a piece of software to be compatible with an ISA, and then expect it to run on any machine that implements that ISA.
