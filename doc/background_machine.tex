...
\begin{itemize}
  \item \texttt{ADD} (\say{add}) Add the values of two registers and store the resulting value in a third register.
  \item \texttt{ADDI} (\say{add immediate}) Add an immediate value to the value of a register and store the resulting value in a second register.
  \item \texttt{MUL} (\say{multiply}) Multiply the values of two registers and store the resulting value in a third register. No immediate variant exist for this operation.
\end{itemize}
...
But, what does it mean to link? %TODO: fill out
