...
\begin{tikzpicture}[squares/.style={align=center, text width=3cm, text=bluetext, minimum width=4cm, minimum height=4cm}]
    ...
    \node[squares,fill=lightblue] (A) at (0,0) {Python\\JavaScript};
    \node[squares,fill=darkblue,anchor=west] (B) at (A.east) {Java\\C\#\\Elixir};
    \node[squares,fill=darkblue,anchor=north] (C) at (A.south){};
    \node[squares,fill=lightblue,anchor=north] (D) at (B.south) {C\\Rust};
    ...
    \node[anchor=east,text=grey,xshift=-2mm] at (A.west) {\rotatebox{90}{Ja}};
    \node[anchor=east,text=grey,xshift=-1cm,align=right] at (A.south west) {\rotatebox{90}{Runtime}};
    \node[anchor=east,text=grey,xshift=-2mm] at (C.west) {\rotatebox{90}{Nej}};
