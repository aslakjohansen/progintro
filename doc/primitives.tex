...
From figure \ref{fig:prim:int:csharp:types} it is evident that \csharp\ lugs around some legacy. Otherwise, they would not have been that inconsistent in the naming. Rust is an example of a modern language, that -- at least when it comes to integer types -- has chosen consistency over history. The equivalent table for Rust can be found in figure \ref{fig:prim:int:rust:types}. Try to compare the different naming conventions.
