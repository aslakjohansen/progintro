...
% set of operations
In order to \textsl{do} anything with our integers, we need to introduce operations on them. Syntax \ref{syntax:prim:arithmetic:ops} shows the syntax for the most commonly used binary operators for integers. Those are mathematical operations that take two input values (called the operands) and produce one output value. These are all infix operators, meaning that the operator is placed in between the two operands.
