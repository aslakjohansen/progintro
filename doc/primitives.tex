...
Adding two floating-point numbers involves finding a common exponent. This may result in reduced resolution in one of the operands, and that causes rounding errors in the result. Because of this, we may find that $a+b=a$ even at times when $b \neq 0$. This usually happens when the magnitude of $a$ and $b$ are very different. Something similar happens with other operations on floating-point numbers, and the compounding error can end up being not only significant but large as well. This is referred to as numerical instability. In scientific computing, care is taken to rephrase important calculations in such ways that the order of operations minimize the introduced error.
