...
In \csharp, variables are first declared, and then used. We can declare a variable and assign it a value on the same line, but the way that the resulting parse tree is evaluated still dictates that the declaration is evaluated first. In Python -- with its weak typing -- local variables are never declared; the moment you assign a value to a name, that name becomes a variable. The following is valid python code:
...
Python doesn't have increment and decrement oprators (e.g., as in \texttt{i++} or \texttt{i += 1}). This is likely to avoid the situation where the veriable has not been initialized and thus does not have a balue to adjust.
