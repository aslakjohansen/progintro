...
\begin{itemize}
  ...
  \descitem{XOR Operator} Whether exactly one of two values is true. It is written as $a \oplus b$. A swinging door will open if it is either being pushed or being pulled: $will\_open = pushed \oplus pulled$.
\end{itemize}
...
% differentiating between OR and XOR: can both inputs be true? (then it doesn't matter and we normally use XOR), are both allowed to be true?, example(game is over if you win or die)
