...
\begin{itemize}
  ...
  \descitem{Biased Exponent:} If the mantissa is zero, then so is the exponent. Otherwise, the exponent represent how much the mantissa needs to be shifted after adding the previously removed initial \say{\texttt{1}}. That is, where to place the comma. In practice, we take the binary logarithm to the positive variant of our number (we have already covered the sign, remember?). We then floor the resulting floating-point number and add a bias. This is our biased exponent. In our example, this is $\lfloor \log_2(1.2) \rfloor + 127 = 127$, and that is \say{\texttt{01111111}} in 8-bit binary which is needed for the single precision variant.
\end{itemize}
