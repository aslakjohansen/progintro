...
Syntax \ref{syntax:prim:cast} introduces the practical mechanics of an explicit cast in \csharp: We need an expression of one type and a different type to cast it to. If the cast is \idx{invalid}{Cast!Invalid} (i.e., not between compatible types) then the \idx{type checker}{Type!Checker} catch it and reject the code with a \idx{cast error}{Cast!Error}. An implicit cast does exactly the same, but the destination type is automatically \idx{inferred}{Inference}, and (ii) it is only allowed when the type conversion is guaranteed to be \idx{lossless}{Lossless!Type conversion}. As it is done automatically, no syntax is needed.
