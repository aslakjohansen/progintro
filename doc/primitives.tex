...
\subsection{Operations on Non-Truth Values}

Any valid comparison between two non-truth values will yield a truth value. For instance, if we ask \quoted{is 42 equal to 56?} then the resulting value is \valuename{false} but if we ask \quoted{is 1 less than or equal to 2?} then the resulting value is \valuename{true}. The full set of comparison operators are listed in figure \ref{fig:prim:bool:comparison}. This is one of the primary ways of creating boolean values.
...
% the role of booleans extend beyond that of representing the result of some comparison, there is a whole algebra for 
