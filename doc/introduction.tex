...
% picking high-level:
As this is an introductory course, we will mostly not care about efficiency. This textbook was written primarily with a software education in mind that is mostly focused on how software is constructed for organizations by essentially gluing existing solutions together. I would call this a \idx{high-level software education}{Software education!High-level}. In such there is little time to learn about programming, so it is important that all the programming courses are directly applicable. For this reason we focus on a high-level language, namely \csharp. This choice, as any other, comes with downsides. So, let's take a look at the main ones:
\begin{itemize}
  \item Once you have learned a language it is easy to go up in \idx{abstraction}{Abstraction} but hard to go down. When you go up in abstraction, there is somethings that you no longer have to worry about. When you go down in abstractions, something that you have come to rely on is taken away. By going the way of a \idx{high-level language}{Language!High-level} in the beginning, makes it likely that you will stay there.
  \item Any \idx{high-level language}{Language!High-level} will have high-level constructs, and why they may at times be easy to use, they tend to be hard to understand. It is simply far away form how the computer works. Unfortunately, that understanding is key to becoming a good programmer. A more ideal approach would be to start with a low-level language, then teach a medium-level language and finally end up with a high-level language. But this all takes time, and on a high-level software education that is unfortunately at a premium.
\end{itemize}

% gen-ai: but is it worth it to learn how to program today (with gen-ai)?, generative ai can be used for rapid prototyping today (yet the resulting codebase typically needs to be thrown away before a product is made from it), prominent stakeholders (on the supply side) have prophesized the irrelevancy of programmers in 6 months (since 2023), yet the ai keeps making serious mistakes in the produced logic and horrible decisions in structure, one needs the brain of skilled programmer to catch it when it goes wrong, that can be a very uninspiring task and as a result mistakes are often made (and that costs time and money) % TODO: add statistics of how often companies benefit from using genai and startup failure rate

\section{Intended Audience}

% background: no programming experience, little understanding of computers, good understanding of internet
This book will make no assumption of previous programming experience. It will, however, expect you to have some minimal understanding of your \idx{operating system}{Operating system} of choice. You should, for instance, know how to install software, and preferably have an understanding of how your \idx{filesystem}{Filesystem} is structured. Section \ref{bg:fs} covers what you need to know about your filesystem, but it won't make up for lack of practical experience. Finally, we expect you to have basic skills in looking up information on the internet and, critically, assessing its relevance and quality.
...
\begin{enumerate}
  ...
  \item Focus should be on concepts to root an understanding in rather than patterns to memorize and and apply.
...
% need for practical experience: when learning programming the need for practical experience is absolutely crucial, you may understand all the examples in the book and agree with all of the solutions to the exercises but that does not make you capable of solving a problem through code and the easiest way -- by far -- to build an understanding of the subject is through repeated use % TODO: not done

% active learning: video < text < doing, but doing requires input (e.g., from text)
...
% ironies of automation: as automation is designed to handle the common cases the onus will be on humans to deal with the special cases and the fallout when the automation fails, but the will not build up the experience and understanding of the systems common cases to be well-equipped to deal with this
\cite{BAINBRIDGE1983775}

% TODO: maybe something about microskills

% TODO: something about taking agency

\section{Societal Responsibilities}

% warning: this section gets fairly dark, but there is no point in deviating from the trajectory of the planet, if we are to avoid a total collapse of the societies that we know today then we need to accept where we are going and actively take steps to change a complex of things, a moonshot is necessary and it will need effort focus and resources like no other project in history
Be advised that his section gets fairly dark. If you are not somewhat informed about the state of the planet before reading this, you will not have a good day. There is no point in having our perception of reality deviate from the trajectory of the \idx{planet}{Planet} though. If we are to avoid a total collapse of the \idx{societies}{Society} that we know today then we need to accept where we are headed and actively take steps to change direction of a complex of issues. A \idx{moonshot}{Moonshot} is necessary, and it will need effort, focus and resources like no other project in \idx{history}{History}.

% the water bankruptcy (data centers, large scale AI): the UN has found that we have moved from a water crisis into a state of global water bankruptcy [citation], this bankruptcy is defines as both insolvency and irreversibility, insolvency because we are withdrawing water faster than it can be replenished and have crossed safe depletion limits, irreversibility because damage both has been done and is being done to the natural mechanisms needed to restore the capacity of our water systems, production of electronics and datacenter operations are major consumers of water, the more they consume the less there is for the part of the human population that can afford it
The \idx{UN}{UN} has found that we have moved from a \idx{water crisis}{Water!Crisis} (which is a temporary thing) into a state of global \idx{water bankruptcy}{Water!Bankruptcy}\cite{WaterBankruptcy26}. This bankruptcy is defined as the combination of insolvency and irreversibility. \idx{Insolvency}{Insolvency}, because we are withdrawing water from the environment faster than it can be replenished and have crossed safe depletion limits. \idx{Irreversibility}{Irreversibility}, because damage both has been done and is being done to the natural mechanisms needed to restore the capacity of our \idx{water systems}{Water!Systems}. Production of electronics and \idx{data center}{Data center} operations are major consumers of water. The more they consume, the less there is for the part of the \idx{human population}{Human!Population} that can afford it. As the \idx{price}{Price} goes up and the \idx{availability}{Availability} down, the part of the population that is too poor for clean drinking water is growing. With explosion in data center construction for \idx{generative AI}{AI!Generative}, many communities in the \idx{USA}{USA} have begun realizing that they might soon turn out to be too poor to compete with a heavily subsidized generative AI industry over clean water.

% climate change: gaia hypothesis, the complex weave of control mechanisms, how we have been weakening them and picking them apart, we are taking away the planets means of self-regulating, current state, risk of a runaway scenario, urgent need for lowering of emissions, the role of software
Under the umbrella term of the \idx{Gaia Hypothesis}{Gaia hypothesis}, \idx{James Lovelock}{Lovelock, James} described the \idx{Earth}{Earth} as a living organism\cite{LOVELOCK1972579}. It is governed by a complex weave of \idx{control mechanisms}{Control mechanism} including the ocean current systems, the winds, the rainforests, and many others. Around the industrial age we slowly began dismantling these control mechanisms. Whenever we eradicate some species that triggers changes in other species which in term causes a \idx{feedback loop}{Feedback loop} to break, and suddenly the effects become significant. The large \idx{coral reefs}{Coral reef} can no longer handle the rising sea temperatures and are going to become a wonder of history. That is not just a loss of beauty, but a massive it to the oceans ability to support life. We are cutting down the \idx{rainforests}{Rainforest} an ever increasing rate. The rainforests plays a major role in regulating the climate, and the are sequesting enormous amounts of carbon. As we replace them with deserts, we loose these capacities and that results in less stable climate conditions, typically of higher temperature. This \idx{climate change}{Climate change} is causing the Atlantic Meridional Overturning Circulation (\idx{AMOC}{AMOC}) oceanic current system to weaken and it is currently approaching a potential collapse. This current is not particularly well understood, but there likely are \idx{tipping points}{Tipping point} that will cause an eventual collapse once passed. The AMOC is one of the great heat distributors of the planet. Without it, the tropics will we warmer and the arctics colder. In short, the bands of livable conditions would become narrower. As the temperature goes up, ice melts and that makes the planet darker which decreases the \idx{albedo}{Albedo}. This in turn causes the planet to absorb more sunlight. The risk of a \idx{runaway scenario}{Runaway scenario} is real, but -- again -- not well understood. Basically, we won't know until it is too late and it might already be. We need to slow down this self-inflicted destruction of the planets mechanisms for sustaining human life. But the ball is already rolling. To stop it we have an urgent need to drastically reduce \idx{emissions}{Emissions}, and software plays a pivotal role here. In part because a lot of the energy consumption is linked to software, in part because software development -- due to the rise of \idx{generative AI}{AI!Generative} -- has a future set out for it that is extremely wasteful, and in part because software can be an enabler to reduce energy consumption in all kinds of other domains.

% the finite planet: the idea of eternal growth on a finite planet is obviously preposterous, and yet growth radical keep pushing societal models that relies on growth, eventually this will fail and whoever is left on the planet will suffer when that happens, given the prohibitive cost of leaving the planet this will amount to the brunt of humanity, current estimates point to the Earth being capable of supporting 500M-3B people given well-thought-out distribution of resources [citations needed], given current trends such a distribution does not seem likely so a practical number is likely quite a bit lower and climate change will lower it even further, nature will take care of the down-regulation that the resulting wars fails to, sure the planet will recover but -- because of the timescale involved -- this may as well be seen a a pure theoretical

% history of efficiency: how people write software has changed quite a bit through the course of the history of modern computing, need (slow hardware), carelessness (moore's law, evolution of performance per price), internet (shifting code to the clients electrical bill), mobile (same), developer price becomes driving parameter of software development, production is messy and power consuming, so are data centers and networks
How people produce software has changed quite a bit through the course of the history of modern computing. In the beginning hardware was expensive, slow and had few resources (e.g., little \idx{memory}{Memory}). This placed a need to produce simple and efficient code on the software developers of that age. As \idx{photolithography}{Photolithography} improved, year over year, more \idx{transistors}{Transistor} could be crammed onto the same piece of silicon at the same price\cite{moore1965}. The observation was that the number of transistors on an \idx{integrated circuit}{Integrated circuit} (i.e., a chip) doubled every two years. This later became known as \idx{Moore's Law}{Moore's Law}, and it meant that programmers had ever increasing processing capabilities to exploit. Programmers got careless with resources, and the time it took to produce the software began to gain importance. When the Internet came, some software was split into a \idx{backend}{Backend} (running on a \idx{server}{Server} on a \idx{centralized}{Centralization} computer) and a \idx{frontend}{Frontend} (running in the \idx{browser}{Browser} of the \idx{client}{Client}). That frontend part was running on client resources and there was thus little incentive to make it efficient. As \idx{mobile devices}{Mobile device} (e.g., \idx{smartphones}{Smartphone}) were introduced this trend continued. Today, developer price has become a primary parameter of software development. With \idx{generative AI}{AI!Generative}, production of software has become messy and \idx{power}{Power} consuming, and so has \idx{data centers}{Data center} and \idx{networks}{Network}. And the code being produced is incredibly inefficient compared to how it all started, but that also means that there are gains to be made.

% privacy: ever since digital encryption became generally available it has been the fulcrum of a fight between privacy and security though legal surveillance, this friction led to the idea of requiring a key escrow system in which the government have access to all decryption keys, this never made wide-spread adoption and people generally felt that their communications were secure, after all the computational cost of breaking common encryption systems like RSA was extreme, however, in 2013 Edward Snowden revealed that western spy agencies had managed to conduct massive surveillance of a large proportion of the planets general population not by breaking the encryption but by compromising dominant hardware and software players through extrajudicial means, the NSA for instance had access to reading peoples emails, while that is bad enough in and of itself who is to say that by now they don't have write access to them? after all there were no serious repercussions from the snowden revelations, and with write access it would likely be trivial to plant evidence

% surveillance and the indifference to using it for mass targeting (politics, ice, palantir, idf): mass surveillance and processing of these such data has become big business, big players like Palantir and various Israeli companies are offering services to (i) influence elections through demographic profiling (ii) target political opponents [ICE] (iii) target the pillars of civil infrastructure [IDF], by now Palantir is paid by lots of western governments to siphon data about their own population and it seems highly unclear which kind of securities are being offered, recently (when?) Switzerland formally rejected and ended its contracts Palantir but it is still gaining power, Alex Karp -- the CEO of palantir -- released a dystopian manifesto in 2025, among others it seeks to subvert the democratic process, the fact that they choose to name their company after the crystal balls in \say{the Lord of the Rings} chat Sauron uses alone should raise a few eyebrows
\cite{karp2025technological}

% news and politics under attack: we are in the misinformation age and have been so for some time now, there is a mismatch of effort between producing seemingly correct information and verifying such information, in other words the balance of power has shifted wildly towards bad actors, as it is obvious that this is the winning strategy more and more actors choose this path, and it turns out that if enough acts are committed then the organs tasked with performing the checks and balances are overwhelmed and they cease to function, it is essentially a denial of service attack

% wars and the international rule of law: and now we are starting to see leaders who doesn't even pretend to care, might makes right, climate wars, morals are becoming expensive

% boom-bust: in other words, we are entering the find-out phase
...
Many \idx{futures}{Future} have the potential to lie ahead of us. The most likely ones are truly dark, but we do have some agency in what will come to pass. Through our actions, we are collectively picking among these potential futures. It is up to our generations to make that decision in the pivotal moment in the story of \idx{humanity}{Humanity}. I challenge you to make better choices than those who came before you.
