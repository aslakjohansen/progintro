...
% picking high-level:
As this is an introductory course, we will mostly not care about efficiency. This textbook was written primarily with a software education in mind that is mostly focused on how software is constructed for organizations by essentially gluing existing solutions together. I would call this a high-level software education. In such there is little time to learn about programming, so it is important that all the programming courses are directly applicable. For this reason we focus on a high-level language, namely \csharp. This choice, as any other, comes with downsides. So, lets take a look at the main ones:
\begin{itemize}
