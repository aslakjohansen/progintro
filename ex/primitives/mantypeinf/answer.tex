...
\begin{enumerate}
  ...
  \item The \texttt{+} operator is defined on a number of types:
    \begin{itemize}
      ...
      \item If both operands are integer types then the resulting value with have an integer type. That type will match the largest of the operand types. This restricts the type of \varname{a} to integers of a size that is no larger than an \typename{int}. Those are \typename{sbyte}, \typename{short} and \typename{int}.
    \end{itemize}
\end{enumerate}
